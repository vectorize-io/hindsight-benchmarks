...
import { RerankerRow } from '@/lib/reranker'
...
// For 0–1 metrics (MRR, Recall), convert to 0–100 for color helpers
...
interface RerankerTableProps {
  rerankers: RerankerRow[]
}

interface TableRow extends RerankerRow {
  fixedRank: number
}


export default function RerankerTable({ rerankers }: RerankerTableProps) {
  const tableData: TableRow[] = useMemo(() => {
    const sorted = [...rerankers].sort((a, b) => b.score.totalScore - a.score.totalScore)
    const rankMap = new Map<string, number>()
    sorted.forEach((r, i) => rankMap.set(r.reranker_id, i + 1))
    return rerankers.map(r => ({ ...r, fixedRank: rankMap.get(r.reranker_id) ?? 999 }))
  }, [rerankers])
  ...
  const columns: ColumnDef<TableRow>[] = [
    ...
    {
      id: 'name',
      accessorFn: row => row.reranker_id,
      header: 'Reranker',
      cell: ({ row }) => (
    ...
    },
    {
      ...
      cell: ({ getValue, row }) => {
        ...
        const { pricePerSearch, pricingType } = row.original.score
        return (
          <div className="relative px-6 py-4" style={{ backgroundColor: scoreToBackground(score) }}>
            <div className="absolute inset-0" style={{ width: `${score}%`, backgroundColor: scoreToBar(score) }} />
            <div className="relative z-10">
              ...
              <div className="text-xs text-muted-foreground">
                {pricingType === 'free' ? 'Free' : `$${pricePerSearch.toFixed(4)}/search`}
              </div>
            </div>
          </div>
        )
